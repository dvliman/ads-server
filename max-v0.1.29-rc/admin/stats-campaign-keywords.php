...
{
	if (isset($Session['prefs']['stats-campaign-keywords.php']['hideinactive']))
		$hideinactive = $Session['prefs']['stats-campaign-keywords.php']['hideinactive'];
	else
...
if (!isset($listorder))
{
	if (isset($Session['prefs']['stats-campaign-keywords.php']['listorder']))
		$listorder = $Session['prefs']['stats-campaign-keywords.php']['listorder'];
	else
		$listorder = '';
}
...
{
	if (isset($Session['prefs']['stats-campaign-keywords.php']['orderdirection']))
		$orderdirection = $Session['prefs']['stats-campaign-keywords.php']['orderdirection'];
	else
		$orderdirection = '';
}

if (isset($Session['prefs']['stats-campaign-keywords.php']['stats_tree']))
	$stats_tree = $Session['prefs']['stats-campaign-keywords.php']['stats_tree'];
else
	$stats_tree = array();
	...
	if (isset($HTTP_POST_VARS['upload']))
	{
		...
		function parseFile($filename, $columnNamesToGet) {
			...
			if (isset($filename) && $filename !='')
			{
				...
				else if (preg_match("/^ÿþK/i", $content))
				{
...
$res_external_data = phpAds_dbQuery("
						SELECT
							resource
						FROM
							".$phpAds_config['tbl_campaign_resources']."
						WHERE
							campaignid = ".$campaignid
); 
...
while ($row_banners = phpAds_dbFetchArray($res_banners))
{
	...
	if(isset($HTTP_POST_VARS[$key]))
		$stats[$banner_index]['total_cost'] = $HTTP_POST_VARS[$key];
	...
	while ($row_clicks = phpAds_dbFetchArray($res_clicks))
	{
		...
		$res_conversions = phpAds_dbQuery("
					...
					FROM
						".$phpAds_config['tbl_conversionlog']."
					WHERE
...
//-----------------------------------------------------------------------------------------------------------------------
	...
	echo "<form name='choose_period' action='".$HTTP_SERVER_VARS['PHP_SELF']."'>";
	echo "<input type='hidden' name ='clientid' value='".$clientid."'>";
	echo "<input type='hidden' name ='campaignid' value='".$campaignid."'>";
	echo "<select name='period' onChange='this.form.submit();' accesskey='".$keyList."' tabindex='".($tabindex++)."'>";
...
if ($clientshidden > 0 || $totalviews > 0 || $totalclicks > 0 || $totalconversions > 0)
{
	...
	// $title column
	echo '<td height="25"><b>&nbsp;&nbsp;<a href="'.$HTTP_SERVER_VARS['PHP_SELF'].'?clientid='.$clientid.'&campaignid='.$campaignid.'&listorder='.$GLOBALS['strName'].'">'.$GLOBALS['strName'].'</a>';
	if ($listorder == $GLOBALS['strName'] || $listorder == "")
		echo $orderdirection == "up" 
			? ('<a href="'.$HTTP_SERVER_VARS['PHP_SELF'].'?clientid='.$clientid.'&orderdirection=down&campaignid='.$campaignid.'"><img src="images/caret-u.gif" border="0" alt="" title=""></a>')
			: ('<a href="'.$HTTP_SERVER_VARS['PHP_SELF'].'?clientid='.$clientid.'&orderdirection=up&campaignid='.$campaignid.'"><img src="images/caret-ds.gif" border="0" alt="" title=""></a>');
	echo '</b></td>';
	...
	if ($listorder == $GLOBALS['strID'])
		echo $orderdirection == "up" 
			? ('<a href="'.$HTTP_SERVER_VARS['PHP_SELF'].'?clientid='.$clientid.'&campaignid='.$campaignid.'&orderdirection=down"><img src="images/caret-u.gif" border="0" alt="" title=""></a>')
			: ('<a href="'.$HTTP_SERVER_VARS['PHP_SELF'].'?clientid='.$clientid.'&campaignid='.$campaignid.'&orderdirection=up"><img src="images/caret-ds.gif" border="0" alt="" title=""></a>');
	echo '</b></td>';
...
$Session['prefs']['stats-campaign-keywords.php']['hideinactive'] 	= $hideinactive;
$Session['prefs']['stats-campaign-keywords.php']['listorder'] 		= $listorder;
$Session['prefs']['stats-campaign-keywords.php']['orderdirection'] 	= $orderdirection;
$Session['prefs']['stats-campaign-keywords.php']['stats_tree'] 		= $stats_tree;
