...
+---------------------------------------------------------------------------+
$Id: config.php 3145 2005-05-20 13:15:01Z andrew $
*/
...
ini_set('include_path', phpAds_path . '/libraries/pear');
...
{
    // For the redirect to work correctly, we have to find the last occurence of /admin/ 
    // Since if Max was installed in a subdirectory, we only need the SSL prefix + /admin/.* for the redirect
    $adminOffset = (strlen($_SERVER['PHP_SELF']) - strlen('/admin/') - strpos(strrev($_SERVER['PHP_SELF']), strrev('/admin/')));
    
    header ('Location: '.$phpAds_config['ssl_url_prefix'].substr($HTTP_SERVER_VARS['PHP_SELF'], $adminOffset));
	exit;
}
...
$phpAds_nav = array (
		...
		"2"							=>  array("stats-global-advertiser.php" => $strStats),
		    ...
		    "2.1.2"					=>  array("stats-advertiser-campaigns.php?clientid=$clientid" => $strCampaignOverview),
			  ...
			  "2.1.2.2"				=> 	array("stats-campaign-banners.php?clientid=$clientid&campaignid=$campaignid" => $strBannerOverview),
    	        ...
    	        "2.1.2.2.2" 		=> 	array("stats-banner-affiliates.php?clientid=$clientid&campaignid=$campaignid&bannerid=$bannerid" => $strPublisherDistribution),
    	      "2.1.2.3"		 		=> 	array("stats-campaign-affiliates.php?clientid=$clientid&campaignid=$campaignid" => $strPublisherDistribution),
		...
		"4"							=>	array("advertiser-index.php" => $strAdminstration),
		  ...
		  "4.2" 					=> 	array("affiliate-index.php" => $strAffiliatesAndZones),
		    ...
		    "4.2.3" 				=> 	array("affiliate-zones.php?affiliateid=$affiliateid" => $strZoneOverview),
  		    ...
  		    "4.2.4"                 =>  array("affiliate-invocation.php?affiliateid=$affiliateid" => $strAffiliateInvocation),
	...
	"client" => array (
		"1"							=>  array("stats-advertiser-history.php?clientid=$clientid" => $strStats),
		  ...
		  "1.2"						=>  array("stats-advertiser-campaigns.php?clientid=$clientid" => $strCampaignOverview),
			...
			"1.2.2"					=> 	array("stats-campaign-banners.php?clientid=$clientid&campaignid=$campaignid" => $strBannerOverview),
    	      ...
    	      "1.2.2.4" 			=> 	array("stats-banner-affiliates.php?clientid=$clientid&campaignid=$campaignid&bannerid=$bannerid" => $strPublisherDistribution),
    	    "1.2.3"		 			=> 	array("stats-campaign-affiliates.php?clientid=$clientid&campaignid=$campaignid" => $strPublisherDistribution),
    	    "1.2.4"					=> 	array("stats-campaign-target.php?clientid=$clientid&campaignid=$campaignid" => $strTargetStats),
 		"3"							=>  array("report-index.php?clientid=$clientid" => $strReports),
    	'2'                         =>  array("advertiser-campaigns.php?clientid=$clientid" => $strAdminstration),
            '2.1'                   =>  array("advertiser-campaigns.php?clientid=$clientid" => $strCampaignOverview),
              '2.1.1'               =>  array("campaign-banners.php?clientid=$clientid&campaignid=$campaignid" => $strBannerOverview),
                '2.1.1.1'           =>  array("banner-edit.php?clientid=$clientid&campaignid=$campaignid&bannerid=$bannerid" => $strBannerProperties),
                '2.1.1.2'           =>  array("banner-acl.php?clientid=$clientid&campaignid=$campaignid&bannerid=$bannerid" => $strModifyBannerAcl),
                '2.1.1.3'           =>  array("banner-append.php?clientid=$clientid&campaignid=$campaignid&bannerid=$bannerid" => $strAppendOthers),
	),
...
if (phpAds_isUser(phpAds_Client)) {
    if (phpAds_isAllowed(phpAds_ModifyInfo))
        $phpAds_nav['client']['4'] = array("advertiser-edit.php?clientid=$clientid" => $strPreferences);
}
?>