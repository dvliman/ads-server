...
+---------------------------------------------------------------------------+
$Id: stats-optimise1.php 3145 2005-05-20 13:15:01Z andrew $
*/
...
if ($campaignid > 0)
{
	...
	if ($campaign_row = phpAds_dbFetchArray($campaign_res))
	{
		...
		$banner_res = phpAds_dbQuery(
			...
			",IF(SUM(views) > 0, SUM(clicks)/SUM(views)*100, 0.00) AS sum_ctr".
			//",IF(SUM(clicks) > 0, SUM(conversions)/SUM(clicks)*100, 0.00) AS sum_cnvr".
			" FROM ".$phpAds_config['tbl_banners'].",".$phpAds_config['tbl_adstats'].
			" WHERE campaignid = ".$campaignid.
			" AND ".$phpAds_config['tbl_banners'].".bannerid=".$phpAds_config['tbl_adstats'].".bannerid".
			" GROUP BY bannerid".
...
function phpAds_buildBannerWhereClause($bannerids)
{
	...
	else
	{
		for ($i=0; $i<sizeof($bannerids); $i++)
		{
			...
			$str .= $phpAds_config['tbl_adstats'].".bannerid=".$bannerids[$i];
		}
	}
	$str .= ")";
