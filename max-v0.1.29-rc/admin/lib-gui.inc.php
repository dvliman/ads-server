...
+---------------------------------------------------------------------------+
$Id: lib-gui.inc.php 3145 2005-05-20 13:15:01Z andrew $
*/
...
function phpAds_PageHeader($ID, $extra="")
{
	...
	if ($ID != phpAds_Login && $ID != phpAds_Error)
	{
		...
		if (phpAds_isLoggedIn() && ( phpAds_isUser(phpAds_Admin) || phpAds_isUser(phpAds_Agency) ) && !defined('phpAds_installing'))
		{
			...
			$searchbar .= "\t\t<form name='search' action='admin-search.php' target='SearchWindow' onSubmit=\"search_window(document.search.keyword.value,'".$phpAds_config['admin_url_prefix']."/admin/admin-search.php'); return false;\">\n";
			...
			$searchbar .= "\t\t\t<td><a href=\"javascript:search_window(document.search.keyword.value,'".$phpAds_config['admin_url_prefix']."/admin/admin-search.php');\"><img src='images/".$phpAds_TextDirection."/go.gif' border='0'></a></td>\n";
			$searchbar .= "\t\t\t<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>\n";
			$searchbar .= "\t\t</tr>\n";
			$searchbar .= "\t\t</form>\n";
			$searchbar .= "\t\t</table>\n";
	...
	echo "\t\t<meta name='generator' content='".$phpAds_productname." ".$phpAds_version_readable." - http://max.awarez.net'>\n";
    ...
    //  added details to display the currently logged on user and IP (D. Turner)
    if (phpAds_isLoggedIn()) {
        echo "<span style='color:#ffffff'><b>" . $Session['username'] . "</b>&nbsp;[" . $_SERVER['REMOTE_ADDR']. "]</span>&nbsp;&nbsp;&nbsp;\n";
    }
	...
	if ($ID != "" && phpAds_isLoggedIn() && !defined('phpAds_installing'))
	{
		if (phpAds_isUser(phpAds_Admin))
		{
			echo "\t\t\t\t<a class='tab-n' href='http://max.awarez.net/' target='_blank'";
			echo "onClick=\"openWindow('http://max.awarez.net/','',";
			echo "'status=yes,menubar=yes,scrollbars=yes,resizable=yes,width=700,height=500'); return false;\">$strHelp</a> \n";
			echo "\t\t\t\t<a href='http://max.awarez.net/' target='_blank'";
			echo "onClick=\"openWindow('http://max.awarez.net/','',";
	...
	if (is_array($array))
	{
		...
		foreach($array as $array)
		{
			...
			else if ($kind == 'banner') 
			{
				...
				if ($isClient)
				{					
					echo "<a href='stats-banner-history.php?clientid=".$id."&bannerid=".$array['id']."&campaignid=".phpAds_getBannerParentCampaignID($array['id'])."'>". ($array['anonymous'] == 't' ? "(Hidden Banner)" : phpAds_getBannerName($array['id'], 30, false)) . "</td>";				
				}
				else
				{
