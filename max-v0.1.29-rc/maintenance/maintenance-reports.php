...
+---------------------------------------------------------------------------+
$Id: maintenance-reports.php 3145 2005-05-20 13:15:01Z andrew $
*/
    ...
    if (phpAds_dbNumRows($res_client) > 0) {
        ...
        if ($client["email"] != '' && $active_campaigns == true) {
            ...
            if (phpAds_sendMail($client['email'], $client['contact'], $Subject, $Body))
            {
                // Update last run
                if ($update == true)
                    $res_update = phpAds_dbQuery("UPDATE ".$conf['table']['clients']." SET reportlastdate=NOW() WHERE clientid=".$client['clientid']);
